...
public class ViewLibraryStudent implements ActionListener {
    JFrame display;
    Container c;
    ...
    public ViewLibraryStudent() {
        ...
        viewstddtltxt = new JLabel("VIEW LIBRARY BOOKS ");
        viewstddtltxt.setBounds(60, 0, 800, 100);
        viewstddtltxt.setForeground(Color.lightGray);
        viewstddtltxt.setFont(new Font("name", Font.BOLD, 45));
        c.add(viewstddtltxt);

        searchbarfld = new Choice();
        searchbarfld.setBounds(60, 130, 250, 30);
        searchbarfld.setForeground(Color.gray);
        searchbarfld.setFont(new Font("name", Font.BOLD, 20));
        c.add(searchbarfld);
        try {
            Connector c = new Connector();
            ResultSet rs = c.s.executeQuery("select * from LibraryPageAdmin");
            while (rs.next()) {
                searchbarfld.add(rs.getString("Bookname"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        searchbtn.setBounds(170, 175, 100, 30);
        ...
        printbtn.setBounds(60, 175, 100, 30);
        ...
        updatebtn.setBounds(405, 175, 100, 30);
        ...
        cancelbtn.setBounds(295, 175, 100, 30);
    ...
    public void actionPerformed(ActionEvent ae) {
        try {
            ...
            if (ae.getSource() == searchbtn) {
                String query = "select * from LibraryPageAdmin where Bookname = '" + searchbarfld.getSelectedItem()
                        + "'";
                try {
                    Connector c = new Connector();
