...
public class ViewFaculityLeaveForm implements ActionListener {
    JFrame display;
    Container c;
    ...
    public ViewFaculityLeaveForm() {
        ...
        viewstddtltxt = new JLabel("VIEW FACULITY LEAVE DETAILS");
        ...
        searchbartxt = new JLabel("Search By EmployeeID :");
        searchbartxt.setBounds(130, 80, 320, 100);
        ...
        searchbarfld.setBounds(460, 115, 150, 30);
        ...
        try {
            ...
            ResultSet rs = c.s.executeQuery("select * from faculityleaveform");
            while (rs.next()) {
                searchbarfld.add(rs.getString("employid"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        ImageIcon ic = new ImageIcon(ClassLoader.getSystemResource("view3.jpg"));
    ...
    public void actionPerformed(ActionEvent ae) {
        try {
            if (ae.getSource() == searchbtn) {
                String query = "select * from faculityleaveform where employid= '" + searchbarfld.getSelectedItem()
                        + "'";
                try {
                    Connector c = new Connector();
